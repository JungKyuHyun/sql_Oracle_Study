...
--turner가 0이 나오는 오류가..
(규현) null값만 잡고 0인 경우는 수정하지 않았으니깐 turner은 그대로 0으로 나오는 거지.
그리고 참고로 nvl2를 사용할 경우가 있으면 decode 함수를 사용하는게 더 좋대~
...
(규현) 똑같지 않은 식인데 결과가 똑같지 않는 이유가 뭘까요...??
